...
public class CardViewApi21Impl implements CardViewImpl {
    ...
    public void initialize(CardViewDelegate cardViewDelegate, Context context, ColorStateList colorStateList, float f16, float f17, float f18) {
        cardViewDelegate.setCardBackground(new RoundRectDrawable(colorStateList, f16));
        View cardView = cardViewDelegate.getCardView();
        cardView.setClipToOutline(true);
        cardView.setElevation(f17);
        setMaxElevation(cardViewDelegate, f18);
    }
    ...
    public void setElevation(CardViewDelegate cardViewDelegate, float f16) {
        cardViewDelegate.getCardView().setElevation(f16);
    }

    @Override // androidx.cardview.widget.CardViewImpl
    public void setMaxElevation(CardViewDelegate cardViewDelegate, float f16) {
        getCardBackground(cardViewDelegate).setPadding(f16, cardViewDelegate.getUseCompatPadding(), cardViewDelegate.getPreventCornerOverlap());
        updatePadding(cardViewDelegate);
    }

    @Override // androidx.cardview.widget.CardViewImpl
    public void setRadius(CardViewDelegate cardViewDelegate, float f16) {
        getCardBackground(cardViewDelegate).setRadius(f16);
    }
