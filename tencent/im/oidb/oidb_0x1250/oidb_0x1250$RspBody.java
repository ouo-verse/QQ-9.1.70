package tencent.im.oidb.oidb_0x1250;

import com.tencent.mobileqq.pb.MessageMicro;
import com.tencent.mobileqq.pb.PBField;
import com.tencent.mobileqq.pb.PBRepeatMessageField;
import com.tencent.mobileqq.pb.PBUInt64Field;
import com.tencent.mobileqq.tianshu.data.BusinessInfoCheckUpdateItem;
import tencent.im.mutualmark.mutualmark$SpecialWordInfo;
import tencent.im.oidb.common.Banner$ActivityBanner;
import tencent.im.oidb.common.GenericCardPB$GenericCard;
import tencent.im.oidb.oidb_0xcf4.oidb_0xcf4$CommonBody;
import tencent.im.oidb.oidb_0xcf4.oidb_0xcf4$CommonGroupChatInfo;
import tencent.im.oidb.oidb_0xcf4.oidb_0xcf4$DateTips;
import tencent.im.oidb.oidb_0xcf4.oidb_0xcf4$DnaInfo;
import tencent.im.oidb.oidb_0xcf4.oidb_0xcf4$EmptyStatusInfo;
import tencent.im.oidb.oidb_0xcf4.oidb_0xcf4$FriendInfo;
import tencent.im.oidb.oidb_0xcf4.oidb_0xcf4$LoveAchievementInfo;
import tencent.im.oidb.oidb_0xcf4.oidb_0xcf4$LoveTreeInfo;
import tencent.im.oidb.oidb_0xcf4.oidb_0xcf4$MutualMarkInfo;
import tencent.im.oidb.oidb_0xcf4.oidb_0xcf4$MutualScore;
import tencent.im.oidb.oidb_0xcf4.oidb_0xcf4$MutualScoreCard;
import tencent.im.oidb.oidb_0xcf4.oidb_0xcf4$NewDnaInfo;
import tencent.im.oidb.oidb_0xcf4.oidb_0xcf4$PrefetchMutualMarkInfo;

/* compiled from: P */
/* loaded from: classes29.dex */
public final class oidb_0x1250$RspBody extends MessageMicro<oidb_0x1250$RspBody> {
    static final MessageMicro.FieldMap __fieldMap__ = MessageMicro.initFieldMap(new int[]{8, 16, 82, 90, 98, 106, 114, 122, 130, 138, 146, 154, 162, 170, 178, 186, 802, 810, 818, BusinessInfoCheckUpdateItem.UIAPPID_XINGQU_BULUO}, new String[]{"uint64_from_uin", "uint64_to_uin", "msg_cur_mutual_mark_info", "msg_friend_info", "msg_common_group_chat_info", "rpt_msg_mutual_mark_info", "rpt_msg_dna_info", "rpt_msg_prefetch_mutual_mark_info", "rpt_msg_data_tips", "rpt_msg_common_rspbody", "rpt_msg_mutual_score", "rpt_msg_mutual_score_card", "msg_love_ach_info", "rpt_new_dna_info", "msg_empty_status", "msg_love_tree_info", "rpt_mutual_mark_state", "rpt_activity_banner", "special_word_info", "generic_cards"}, new Object[]{0L, 0L, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null}, oidb_0x1250$RspBody.class);
    public final PBUInt64Field uint64_from_uin = PBField.initUInt64(0);
    public final PBUInt64Field uint64_to_uin = PBField.initUInt64(0);
    public oidb_0xcf4$MutualMarkInfo msg_cur_mutual_mark_info = new oidb_0xcf4$MutualMarkInfo();
    public oidb_0xcf4$FriendInfo msg_friend_info = new oidb_0xcf4$FriendInfo();
    public oidb_0xcf4$CommonGroupChatInfo msg_common_group_chat_info = new oidb_0xcf4$CommonGroupChatInfo();
    public final PBRepeatMessageField<oidb_0xcf4$MutualMarkInfo> rpt_msg_mutual_mark_info = PBField.initRepeatMessage(oidb_0xcf4$MutualMarkInfo.class);
    public final PBRepeatMessageField<oidb_0xcf4$DnaInfo> rpt_msg_dna_info = PBField.initRepeatMessage(oidb_0xcf4$DnaInfo.class);
    public final PBRepeatMessageField<oidb_0xcf4$PrefetchMutualMarkInfo> rpt_msg_prefetch_mutual_mark_info = PBField.initRepeatMessage(oidb_0xcf4$PrefetchMutualMarkInfo.class);
    public final PBRepeatMessageField<oidb_0xcf4$DateTips> rpt_msg_data_tips = PBField.initRepeatMessage(oidb_0xcf4$DateTips.class);
    public final PBRepeatMessageField<oidb_0xcf4$CommonBody> rpt_msg_common_rspbody = PBField.initRepeatMessage(oidb_0xcf4$CommonBody.class);
    public oidb_0xcf4$MutualScore rpt_msg_mutual_score = new oidb_0xcf4$MutualScore();
    public oidb_0xcf4$MutualScoreCard rpt_msg_mutual_score_card = new oidb_0xcf4$MutualScoreCard();
    public oidb_0xcf4$LoveAchievementInfo msg_love_ach_info = new oidb_0xcf4$LoveAchievementInfo();
    public final PBRepeatMessageField<oidb_0xcf4$NewDnaInfo> rpt_new_dna_info = PBField.initRepeatMessage(oidb_0xcf4$NewDnaInfo.class);
    public oidb_0xcf4$EmptyStatusInfo msg_empty_status = new oidb_0xcf4$EmptyStatusInfo();
    public oidb_0xcf4$LoveTreeInfo msg_love_tree_info = new oidb_0xcf4$LoveTreeInfo();
    public oidb_0x1250$MutualMarkList rpt_mutual_mark_state = new oidb_0x1250$MutualMarkList();
    public final PBRepeatMessageField<Banner$ActivityBanner> rpt_activity_banner = PBField.initRepeatMessage(Banner$ActivityBanner.class);
    public mutualmark$SpecialWordInfo special_word_info = new mutualmark$SpecialWordInfo();
    public final PBRepeatMessageField<GenericCardPB$GenericCard> generic_cards = PBField.initRepeatMessage(GenericCardPB$GenericCard.class);
}
