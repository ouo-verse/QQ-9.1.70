...
public class HippyRecyclerPagerViewController<HRW extends HippyRecyclerViewWrapper> extends HippyViewController<HRW> {
    ...
    public static HippyRecyclerView initDefault(Context context, HippyMap hippyMap, HippyRecyclerPagerView hippyRecyclerPagerView) {
        HippyPagerLinearLayoutManager hippyPagerLinearLayoutManager = new HippyPagerLinearLayoutManager(context);
        hippyRecyclerPagerView.setItemAnimator(null);
        boolean z16 = false;
        if (hippyMap != null) {
            if (hippyMap.containsKey("horizontal")) {
                hippyPagerLinearLayoutManager.setOrientation(0);
            }
            z16 = hippyMap.getBoolean(HippyScrollViewEventHelper.EVENT_TYPE_SCROLL);
        }
        hippyRecyclerPagerView.setLayoutManager(hippyPagerLinearLayoutManager);
        hippyRecyclerPagerView.setHippyEngineContext(((HippyInstanceContext) context).getEngineContext());
        hippyRecyclerPagerView.initRecyclerView();
        hippyRecyclerPagerView.getRecyclerViewEventHelper().setOnScrollEventEnable(z16);
        return hippyRecyclerPagerView;
    }
    ...
    @HippyControllerProps(defaultNumber = 200.0d, defaultType = "number", name = "autoPageScrollDuration")
    public void setAutoPageScrollDuration(HRW hrw, int i3) {
        HippyRecyclerView recyclerView = hrw.getRecyclerView();
        if (i3 > 0 && (recyclerView instanceof HippyRecyclerPagerView)) {
            ((HippyRecyclerPagerView) recyclerView).setAutoPageScrollDuration(i3);
        }
    }
    ...
    @HippyControllerProps(defaultType = "map", name = "itemViewPoolSize")
    public void setItemViewPoolSize(HRW hrw, HippyMap hippyMap) {
        HippyRecyclerView recyclerView = hrw.getRecyclerView();
        if (hippyMap != null && (recyclerView instanceof HippyRecyclerPagerView)) {
            ((HippyRecyclerPagerView) recyclerView).setItemViewPoolSize(hippyMap);
        }
    }
    ...
    @HippyControllerProps(defaultNumber = 0.5d, defaultType = "number", name = "pageUpDownOffsetRatio")
    public void setPageUpDownOffsetRatio(HRW hrw, float f16) {
        HippyRecyclerView recyclerView = hrw.getRecyclerView();
        if (recyclerView instanceof HippyRecyclerPagerView) {
            ((HippyRecyclerPagerView) recyclerView).setPageUpDownOffsetRatio(f16);
        }
    }

    @HippyControllerProps(defaultNumber = 0.0d, defaultType = "number", name = "preCreateRowsNumber")
    public void setPreCreateRowsNumber(HRW hrw, int i3) {
        HippyRecyclerView recyclerView = hrw.getRecyclerView();
        if (recyclerView instanceof HippyRecyclerPagerView) {
            ((HippyRecyclerPagerView) recyclerView).setPreCreateRowsNumber(i3);
        }
    }
    ...
    protected View createViewImpl(Context context, HippyMap hippyMap) {
        HippyRecyclerView initDefault = initDefault(context, hippyMap, new HippyRecyclerPagerView(context));
    ...
    public void dispatchFunction(HRW hrw, String str, HippyArray hippyArray) {
        super.dispatchFunction((HippyRecyclerPagerViewController<HRW>) hrw, str, hippyArray);
        str.hashCode();
        char c16 = '\uffff';
        switch (str.hashCode()) {
            case -1101593308:
    ...
    public void onAfterUpdateProps(HRW hrw) {
        super.onAfterUpdateProps((HippyRecyclerPagerViewController<HRW>) hrw);
        hrw.getRecyclerView().onAfterUpdateProps();
    }

    @Override // com.tencent.mtt.hippy.uimanager.HippyViewController
    public void onBatchComplete(HRW hrw) {
        super.onBatchComplete((HippyRecyclerPagerViewController<HRW>) hrw);
        hrw.onBatchComplete();
        hrw.setListData();
    }
    ...
    public void onBatchStart(HRW hrw) {
        super.onBatchStart((HippyRecyclerPagerViewController<HRW>) hrw);
        hrw.onBatchStart();
    }
