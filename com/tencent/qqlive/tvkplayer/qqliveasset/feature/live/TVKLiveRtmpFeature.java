...
public class TVKLiveRtmpFeature extends TVKLivePlayerFeatureBase {
    public TVKLiveRtmpFeature(TVKContext tVKContext) {
        super(tVKContext);
    }
    ...
    public void buildLiveCGIParams(@NonNull TVKCGIRequestParam tVKCGIRequestParam, ITVKFeatureParamGroup iTVKFeatureParamGroup, @NonNull Map<String, String> map) {
        if (!TVKMediaPlayerConfig.PlayerConfig.enable_live_rtmp) {
            return;
        }
        if (tVKCGIRequestParam.getStreamFormatId() != 4 && tVKCGIRequestParam.getStreamFormatId() != 0) {
            return;
        }
        if (tVKCGIRequestParam.getRequestSource() != 3 && !TVKLivePlayerFeatureBase.isSelfPlayerAvailableForLive()) {
            return;
        }
        addFormatCapabilities(map, 4);
    }
    ...
    public ITVKPlayerFeature.FeaturePlayerAndDecoderChoosePriority getPlayerAndDecoderChoosePriority() {
        return ITVKPlayerFeature.FeaturePlayerAndDecoderChoosePriority.PLAYER_AND_DECODER_CHOOSE_RTMP;
    }
    ...
    public TVKStrategyEnum.ProxyStrategy getProxyStrategy() {
        return TVKStrategyEnum.ProxyStrategy.PROXY_STRATEGY_MUST_NONE;
    }

    @Override // com.tencent.qqlive.tvkplayer.qqliveasset.feature.api.ITVKPlayerFeature
    public ITVKPlayerFeature.FeatureRetryPriority getRetryPriority() {
        return ITVKPlayerFeature.FeatureRetryPriority.RETRY_RTMP;
    }
    ...
    public boolean isMatchFeature(TVKNetVideoInfo tVKNetVideoInfo) {
        if ((tVKNetVideoInfo instanceof TVKLiveVideoInfo) && ((TVKLiveVideoInfo) tVKNetVideoInfo).getStream() == 4) {
            return true;
        }
        return false;
    }
    ...
    public boolean isNeedDisableFeatureForRetry() {
        return TVKMediaPlayerConfig.PlayerConfig.enable_live_rtmp_downward_retry;
    }
