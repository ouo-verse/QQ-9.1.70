...
public class TVKLiveDolbyAudioFeature extends TVKLivePlayerFeatureBase {
    public TVKLiveDolbyAudioFeature(TVKContext tVKContext) {
        super(tVKContext);
    }
    ...
    public void buildLiveCGIParams(@NonNull TVKCGIRequestParam tVKCGIRequestParam, ITVKFeatureParamGroup iTVKFeatureParamGroup, @NonNull Map<String, String> map) {
        if (TVKMediaPlayerConfig.PlayerConfig.is_support_dolby_audio && TVKTPCapability.isSupportDolbyAudio()) {
            addSpaudioCapabilities(map, 2, 4, 8);
        }
    }
    ...
    public boolean isMatchFeature(TVKNetVideoInfo tVKNetVideoInfo) {
        ...
        int audioCodec = tVKNetVideoInfo.getCurDefinition().getAudioCodec();
        if (audioCodec == 2) {
            z16 = true;
        } else {
            z16 = false;
