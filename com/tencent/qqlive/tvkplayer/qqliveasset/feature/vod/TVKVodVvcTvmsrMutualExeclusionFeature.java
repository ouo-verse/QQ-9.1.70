...
public class TVKVodVvcTvmsrMutualExeclusionFeature extends TVKVodPlayerFeatureBase {
    private static final int SUPER_RESOLUTION_UNSUPPORTED = 0;

    public TVKVodVvcTvmsrMutualExeclusionFeature(TVKContext tVKContext) {
        super(tVKContext);
    }
    ...
    public void processOnCGISuccess(@NonNull TVKPlayerInputParam tVKPlayerInputParam, @NonNull TVKPlayerRuntimeParam tVKPlayerRuntimeParam) {
        if (tVKPlayerRuntimeParam.getNetVideoInfo() != null && tVKPlayerRuntimeParam.getNetVideoInfo().getCurDefinition() != null && TVKMediaPlayerConfig.PlayerConfig.enable_vod_vvc_shd_tvmsr_mutual_execlusion && "shd".equals(tVKPlayerRuntimeParam.getNetVideoInfo().getCurDefinition().getDefn()) && tVKPlayerRuntimeParam.getNetVideoInfo().isVvc()) {
            tVKPlayerRuntimeParam.getNetVideoInfo().getCurDefinition().setVideoSuperResolutionType(0);
        }
    }
