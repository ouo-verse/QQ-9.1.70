...
public class TVKVodHdrEnhanceFeature extends TVKVodPlayerFeatureBase {
    private static final int SPVIDEO_HDR10_ENHANCE = 128;

    public TVKVodHdrEnhanceFeature(TVKContext tVKContext) {
        super(tVKContext);
    }
    ...
    public void buildVodCGIParams(@NonNull TVKCGIRequestParam tVKCGIRequestParam, ITVKFeatureParamGroup iTVKFeatureParamGroup, @NonNull Map<String, String> map) {
        boolean z16;
        if (TVKMediaPlayerConfig.PlayerConfig.hdr_enable && TVKMediaPlayerConfig.PlayerConfig.hdr_enhance_decode_enable) {
            z16 = true;
        } else {
            z16 = false;
        }
        boolean isSupportHighQualityVideo = TVKCapabilityMgr.getInstance().isSupportHighQualityVideo(2, z16);
        this.mLogger.info("CGI: supportHDREnhance=" + isSupportHighQualityVideo, new Object[0]);
        if (isSupportHighQualityVideo) {
            addSpvideoCapability(map, 128);
        }
    }
    ...
    public boolean isMatchFeature(TVKNetVideoInfo tVKNetVideoInfo) {
        ...
        if (!"hdr10".equals(curDefinition.getDefn()) || curDefinition.getHdr10EnHance() != 1) {
            return false;
        }
        return true;
    }
