...
public class TVKVodWidevineDrmFeature extends TVKVodPlayerFeatureBase {
    ...
    public void buildVodCGIParams(@NonNull TVKCGIRequestParam tVKCGIRequestParam, ITVKFeatureParamGroup iTVKFeatureParamGroup, @NonNull Map<String, String> map) {
        boolean z16;
        int i3;
        if (TVKMediaPlayerConfig.PlayerConfig.widevine_drm_enable && !(tVKCGIRequestParam.getVideoInfo().getAsset() instanceof TVKOnlineSimulatedLiveAsset)) {
            z16 = true;
        } else {
            z16 = false;
        }
        boolean isSupportDrm = TVKCapabilityMgr.getInstance().isSupportDrm(4, z16);
        ...
        if (isSupportDrm) {
            map.put(TVKCommonParamEnum.REQ_PARAM_KEY_DRM, String.valueOf(TVKUtils.optInt(map.get(TVKCommonParamEnum.REQ_PARAM_KEY_DRM), 0) | 32));
            if (Build.VERSION.SDK_INT >= 24 && TVKMediaPlayerConfig.PlayerConfig.enable_multi_drm_for_widevine) {
                map.put(TVKCommonParamEnum.REQ_PARAM_KEY_MULTI_DRM, String.valueOf(TVKUtils.optInt(map.get(TVKCommonParamEnum.REQ_PARAM_KEY_MULTI_DRM), 0) | 32));
            }
        }
        if ((TVKMediaPlayerConfig.PlayerConfig.vod_cgi_ea_set & 2) == 0) {
            map.put(TVKCommonParamEnum.REQ_PARAM_KEY_CKCVER, String.valueOf(1));
        }
    }
