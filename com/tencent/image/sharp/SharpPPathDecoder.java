...
public class SharpPPathDecoder {
    ...
    public long createDecoder() {
        ...
        if (this.mInfo == null && (parseHeader = parseHeader()) != 0) {
            URLDrawable.depImp.mLog.e(TAG, 2, "createDecoder error,res=" + parseHeader);
            return parseHeader;
        }
    ...
    public Bitmap decodeSharpP2GifFrame(long j3, int i3, int i16, int i17, SharpImage.WriteableInteger writeableInteger, Bitmap bitmap, boolean z16) {
        ...
        if (this.mDecoder.decodeOneFrameInNative(j3, i3, bitmap2, writeableInteger, z16) != 0) {
            return null;
        }
        return bitmap2;
    }
        ...
        if (imageMode != 0) {
            if (imageMode != 1) {
                if (imageMode != 2) {
                    ...
                    URLDrawable.depImp.mLog.e(TAG, 2, "-------unknow mimeType:" + this.mInfo.getImageMode());
                    return null;
                }
            } else {
                return "image/png";
    ...
    public int parseHeader() {
        ...
        if (parseHeaderInNative != 0) {
            this.mInfo = null;
            URLDrawable.depImp.mLog.e(TAG, 2, "parseHeader error,res=" + parseHeaderInNative);
        }
