...
public class t implements com.tencent.imcore.message.core.codec.d<AppInterface> {
    ...
    public int a() {
        ...
        return MessageHandlerConstants.NOTIFY_TYPE_MOVIE_TICKET_SEND_ERROR_RSP;
    }
    ...
    public boolean c(msg_svc$RoutingHead msg_svc_routinghead, MessageRecord messageRecord, AppInterface appInterface) {
        ...
        msg_svc_commtmp.svr_type.set(153);
        msg_svc_routinghead.comm_tmp.set(msg_svc_commtmp);
        byte[] L0 = ((QQAppInterface) appInterface).getMsgCache().L0(messageRecord.frienduin);
        if (L0 != null) {
            if (QLog.isDevelopLevel()) {
                QLog.d("MovieTicketTmpRoutingType", 2, "movieTicket------>" + HexUtil.bytes2HexStr(L0) + ",length:" + L0.length);
            }
            msg_svc_commtmp.sig.set(ByteStringMicro.copyFrom(L0));
        }
        msg_svc_routinghead.comm_tmp.set(msg_svc_commtmp);
        return true;
    }
