...
public final class r extends h {
    ...
    private final void s(View view) {
        Map mapOf;
        int i3 = 2;
        if (((ChannelSettingMviContext) getMContext()).getParams().getChannelInfo().getVisibleType() != 2) {
            i3 = 1;
        }
        mapOf = MapsKt__MapsJVMKt.mapOf(TuplesKt.to("sgrp_channel_set_authority", Integer.valueOf(i3)));
        ChannelSettingUtils.i(view, "em_sgrp_channel_set_access_member", mapOf);
    }
    ...
    */
    public void p(@NotNull View view) {
        ...
        if (((ChannelSettingMviContext) getMContext()).getParams().getChannelInfo().getType() == 7) {
            ...
            if (((Boolean) pair.getFirst()).booleanValue()) {
                ChannelSettingUtils.j(view, "em_sgrp_view_permission_set", null, 4, null);
                if (((ChannelSettingMviContext) getMContext()).getParams().getChannelInfo().getType() != 5 && ((ChannelSettingMviContext) getMContext()).getParams().getChannelInfo().getLiveRoomId() > 0) {
                ...
                if (context == null) {
                    ...
                    if (channelSettingUtils.e(((ChannelSettingMviContext) getMContext()).getParams().getChannelInfo())) {
                        ...
                        GuildOpenBindPermissionUtils.w(context, ((ChannelSettingMviContext) getMContext()).getParams().getChannelInfo());
                        return;
                    }
                }
                return;
        ...
        s(view);
        if (((ChannelSettingMviContext) getMContext()).getParams().getChannelInfo().getType() != 5) {
        }
        context = ((ChannelSettingMviContext) getMContext()).c().getContext();
        if (context == null) {
        }
    ...
    public void q(@NotNull ChannelSettingMviContext context) {
        ...
        int visibleType = context.getParams().getChannelInfo().getVisibleType();
        if (visibleType != 1) {
            if (visibleType != 2) {
                desc = "";
            } else {
        ...
        } else if (((IGuildRoleGroupApi) QRoute.api(IGuildRoleGroupApi.class)).getVisibleForVisitor(context.getParams().getGuildInfo()) && context.getParams().getChannelInfo().getType() != 6) {
            desc = HardCodeUtil.qqStr(R.string.f139550bk);
        } else {
    ...
    public void r(@NotNull dx permissionInfo) {
        ...
        updateUI(new ChannelSettingUIState.SettingItemEnableUIState(permissionInfo.a(IPermissionType.MODIFY_CHANNEL_VIEW)));
    }
}
