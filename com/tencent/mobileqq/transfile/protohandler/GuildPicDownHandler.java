...
public class GuildPicDownHandler extends PicDownHandler {
    static IPatchRedirector $redirector_ = null;
    private static final String TAG = "GuildPicDownHandler.<gld>";

    public GuildPicDownHandler() {
    ...
    private String fileType2LogStr(int i3) {
        if (i3 != 1) {
            if (i3 != 65537) {
                if (i3 != 131075) {
                    return "UNKNOWN" + i3;
        ...
        return "BIG_COMPRESS" + i3;
    }

    private void handleResponse(RichProto.RichProtoReq.GuildPicDownReq guildPicDownReq, cmd0x388$GetImgUrlRsp cmd0x388_getimgurlrsp, RichProto.RichProtoResp.PicDownResp picDownResp) throws UnknownHostException {
        String str;
        int i3 = guildPicDownReq.fileType;
        if (i3 == 65537) {
        ...
        QLog.i(TAG, 1, "step:handleSucResponse urlPath:" + picDownResp.urlPath + " type:" + fileType2LogStr(guildPicDownReq.fileType));
    }

    void addOne(int i3, RichProto.RichProtoReq.ReqCommon reqCommon, cmd0x388$ReqBody cmd0x388_reqbody) {
        RichProto.RichProtoReq.GuildPicDownReq guildPicDownReq = (RichProto.RichProtoReq.GuildPicDownReq) reqCommon;
        ...
        try {
            cmd0x388_getimgurlreq.uint64_dst_uin.set(Long.parseLong(guildPicDownReq.selfUin));
            cmd0x388_getimgurlreq.uint64_qqmeet_channel_id.set(Long.parseLong(guildPicDownReq.peerUin));
            cmd0x388_getimgurlreq.uint64_qqmeet_guild_id.set(Long.parseLong(guildPicDownReq.secondUin));
            cmd0x388_getimgurlreq.bytes_download_index.set(ByteStringMicro.copyFrom(guildPicDownReq.index));
            cmd0x388_getimgurlreq.uint64_group_code.set(Long.parseLong(guildPicDownReq.peerUin));
            cmd0x388_getimgurlreq.bytes_file_md5.set(ByteStringMicro.copyFrom(guildPicDownReq.md5));
        } catch (Exception e16) {
            QLog.i(TAG, 1, "addOne: Exception = " + e16);
        }
        cmd0x388_getimgurlreq.uint32_pic_up_timestamp.set(guildPicDownReq.msgTime);
        long j3 = guildPicDownReq.groupFileID;
        if (j3 != 0) {
            cmd0x388_getimgurlreq.uint64_fileid.set(j3);
        }
        int i16 = guildPicDownReq.fileType;
        int i17 = 2;
        if (i16 == 65537) {
            cmd0x388_getimgurlreq.uint32_url_flag.set(4);
        } else if (i16 == 1) {
        ...
        if (guildPicDownReq.uinType == 10014) {
            i17 = 211;
        }
        pBUInt32Field.set(i17);
        cmd0x388_getimgurlreq.bytes_build_ver.set(ByteStringMicro.copyFromUtf8(TransFileUtil.getVersionCode()));
        cmd0x388_getimgurlreq.uint32_req_transfer_type.set(guildPicDownReq.protocolType);
        cmd0x388_reqbody.rpt_msg_getimg_url_req.add(cmd0x388_getimgurlreq);
        QLog.i(TAG, 1, "step:GetImgUrlReq index:" + cmd0x388_getimgurlreq.bytes_download_index.get().toStringUtf8() + " fileType:" + fileType2LogStr(guildPicDownReq.fileType));
    }
        ...
        if (list != null && list.size() != 0) {
            for (cmd0x388$GetImgUrlRsp cmd0x388_getimgurlrsp : list) {
                try {
                    ...
                    try {
                        RichProto.RichProtoReq.GuildPicDownReq guildPicDownReq = (RichProto.RichProtoReq.GuildPicDownReq) richProtoReq.reqs.get(i17);
                        if (picDownResp3 != null) {
                            ...
                            if (i18 == 0) {
                                handleResponse(guildPicDownReq, cmd0x388_getimgurlrsp, picDownResp3);
    ...
    public void sendRichProtoReq(RichProto.RichProtoReq richProtoReq) {
        ...
        if (richProtoReq != null && richProtoReq.reqs != null && richProtoReq.protoReqMgr != null) {
            ...
            protoReq.ssoCmd = RichMediaConstants.CMD_LONGCONN_GUILDPIC_DOWN;
