...
public class GetReactiveFriendListServlet extends MiniAppAbstractServlet {
        ...
        try {
            ...
            if (fromServiceMsg != null) {
                ...
                if (fromServiceMsg.isSuccess()) {
                    bundle.putParcelable("getReactiveFriendList", fromServiceMsg);
                    bundle.putLong("retCode", stQWebRsp.retCode.get());
                    bundle.putString("errMsg", stQWebRsp.errMsg.get().toStringUtf8());
                    notifyObserver(intent, 1084, true, bundle, MiniAppObserver.class);
                } else {
                    if (QLog.isColorLevel()) {
                        QLog.d(TAG, 2, "onReceive. getReactiveFriendList rsp = " + stQWebRsp);
                    }
                    notifyObserver(intent, 1084, false, bundle, MiniAppObserver.class);
                }
            } else {
                if (QLog.isColorLevel()) {
                    QLog.d(TAG, 2, "onReceive. inform getReactiveFriendList resultcode fail.");
                }
                notifyObserver(intent, 1084, false, bundle, MiniAppObserver.class);
            }
        } catch (Throwable th5) {
    ...
    public void onSend(Intent intent, Packet packet) {
        ...
        byte[] encode = new GetReactiveFriendListRequest(stCommonExt, stringExtra).encode(intent, this.index, getTraceId());
        if (encode == null) {
            encode = new byte[4];
        }
        packet.setSSOCommand("LightAppSvc.mini_app_cloudstorage.GetReactiveFriendList");
