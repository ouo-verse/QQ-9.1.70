...
public class GetUserInteractiveStorageServlet extends MiniAppAbstractServlet {
        ...
        try {
            ...
            if (fromServiceMsg != null) {
                ...
                if (fromServiceMsg.isSuccess()) {
                    bundle.putParcelable("getUserInteractiveStorage", fromServiceMsg);
                    bundle.putLong("retCode", stQWebRsp.retCode.get());
                    bundle.putString("errMsg", stQWebRsp.errMsg.get().toStringUtf8());
                    notifyObserver(intent, 1059, true, bundle, MiniAppObserver.class);
                } else {
                    if (QLog.isColorLevel()) {
                        QLog.d(TAG, 2, "onReceive. getUserInteractiveStorage rsp = " + stQWebRsp);
                    }
                    notifyObserver(intent, 1059, false, bundle, MiniAppObserver.class);
                }
            } else {
                if (QLog.isColorLevel()) {
    ...
    public void onSend(Intent intent, Packet packet) {
        ...
        byte[] encode = new GetUserInteractiveStorageRequest(stCommonExt, stringExtra, stringArrayExtra).encode(intent, this.index, getTraceId());
        if (encode == null) {
            encode = new byte[4];
        }
        packet.setSSOCommand(GetUserInteractiveStorageRequest.CMD_STRING);
