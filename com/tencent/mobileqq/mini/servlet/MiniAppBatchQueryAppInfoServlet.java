...
public class MiniAppBatchQueryAppInfoServlet extends MiniAppAbstractServlet {
        ...
        try {
            ...
            if (fromServiceMsg != null) {
                if (fromServiceMsg.isSuccess()) {
                    ...
                    bundle.putParcelable("batchQueryAppInfo", fromServiceMsg);
                    notifyObserver(intent, 1013, true, bundle, MiniAppObserver.class);
                } else {
                    bundle.putLong("ret_code", fromServiceMsg.getBusinessFailCode());
                    bundle.putString("err_msg", fromServiceMsg.getBusinessFailMsg());
                    notifyObserver(intent, 1013, false, bundle, MiniAppObserver.class);
                }
            } else {
                if (QLog.isColorLevel()) {
                    QLog.d(TAG, 2, "onReceive. inform MiniAppBatchQueryAppInfoServlet resultcode fail.");
                }
                notifyObserver(intent, 1013, false, bundle, MiniAppObserver.class);
            }
        } catch (Throwable th5) {
    ...
    public void onSend(Intent intent, Packet packet) {
        ...
        ArrayList<String> stringArrayListExtra = intent.getStringArrayListExtra("key_appid_list");
        ...
        byte[] encode = new BatchQueryAppInfoRequest(stCommonExt, stringArrayListExtra).encode(intent, intExtra, getTraceId());
        if (encode == null) {
            encode = new byte[4];
        }
        packet.setSSOCommand(BatchQueryAppInfoRequest.CMD_STRING);
