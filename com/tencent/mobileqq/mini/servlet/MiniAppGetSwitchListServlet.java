...
public class MiniAppGetSwitchListServlet extends MiniAppAbstractServlet {
        ...
        try {
            ...
            if (fromServiceMsg != null) {
                ...
                if (fromServiceMsg.isSuccess()) {
                    bundle.putParcelable(KEY_GET_SWITCH_LIST, fromServiceMsg);
                    bundle.putLong("retCode", stQWebRsp.retCode.get());
                    bundle.putString("errMsg", stQWebRsp.errMsg.get().toStringUtf8());
                    notifyObserver(intent, 1049, true, bundle, MiniAppObserver.class);
                } else {
                    if (QLog.isColorLevel()) {
                        QLog.d(TAG, 2, "onReceive. MiniAppGetSwitchListServlet rsp = " + stQWebRsp);
                    }
                    notifyObserver(intent, 1049, false, bundle, MiniAppObserver.class);
                }
            } else {
                if (QLog.isColorLevel()) {
                    QLog.d(TAG, 2, "onReceive. inform MiniAppGetSwitchListServlet resultcode fail.");
                }
                notifyObserver(intent, 1049, false, bundle, MiniAppObserver.class);
            }
        } catch (Throwable th5) {
    ...
    public void onSend(Intent intent, Packet packet) {
        ...
        byte[] encode = new MiniAppGetSwitchListRequest(stCommonExt).encode(intent, this.index, getTraceId());
        if (encode == null) {
            encode = new byte[4];
        }
        packet.setSSOCommand(MiniAppGetSwitchListRequest.CMD_STRING);
