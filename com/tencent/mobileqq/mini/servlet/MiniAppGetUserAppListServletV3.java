...
public class MiniAppGetUserAppListServletV3 extends MiniAppAbstractServlet {
    public static final String KEY_EXT_INFO = "key_ext_info";
    public static final String KEY_GET_USER_APP_LIST_V3 = "getUserAppListV3";
    ...
    public static final String TAG = "MiniAppGetUserAppListServletV3";

    public MiniAppGetUserAppListServletV3() {
        this.observerId = 1105;
    }
    ...
    private COMM.StCommonExt buildExtInfo(byte[] bArr, int i3) {
        ...
        if (bArr != null) {
            try {
                ...
                QLog.e(TAG, 1, "[MiniAppGetUserAppListServletV3] onSend. exception=" + e16.getMessage());
            }
        }
        ...
        try {
            ...
            if (fromServiceMsg != null) {
                if (fromServiceMsg.isSuccess()) {
                    ...
                    bundle.putParcelable(KEY_GET_USER_APP_LIST_V3, fromServiceMsg);
                    notifyObserver(intent, 1105, true, bundle, MiniAppObserver.class);
                } else {
                    bundle.putLong("retCode", fromServiceMsg.getBusinessFailCode());
                    bundle.putString("errMsg", fromServiceMsg.getBusinessFailMsg());
                    notifyObserver(intent, 1105, false, bundle, MiniAppObserver.class);
                }
            } else {
                QLog.e(TAG, 1, "onReceive. inform MiniAppGetUserAppListServletV3 response is null.");
            }
        } catch (Throwable th5) {
    ...
    public void onSend(Intent intent, Packet packet) {
        ...
        byte[] encode = new GetUserAppListRequestV3(buildExtInfo(intent.getByteArrayExtra("key_ext_info"), intent.getIntExtra("scene", 0)), arrayList, intent.getIntegerArrayListExtra("key_module_types"), intent.getIntExtra("key_use_cache", 0), AdUtils.getDeviceInfo(BaseApplication.getContext()), intent.getIntegerArrayListExtra("key_sub_types"), intent.getStringArrayListExtra("key_item_ids")).encode(intent, intExtra, getTraceId());
        if (encode == null) {
            encode = new byte[4];
        }
        packet.setSSOCommand(GetUserAppListRequestV3.CMD_STRING);
