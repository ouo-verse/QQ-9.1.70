...
public class MiniAppSetUserSwitchServlet extends MiniAppAbstractServlet {
        ...
        try {
            ...
            if (fromServiceMsg != null) {
                ...
                if (fromServiceMsg.isSuccess()) {
                    bundle.putParcelable(KEY_SET_USER_SWITCH, fromServiceMsg);
                    bundle.putLong("retCode", stQWebRsp.retCode.get());
                    bundle.putString("errMsg", stQWebRsp.errMsg.get().toStringUtf8());
                    notifyObserver(intent, 1050, true, bundle, MiniAppObserver.class);
                } else {
                    if (QLog.isColorLevel()) {
                        QLog.d(TAG, 2, "onReceive. MiniAppSetUserSwitchServlet rsp = " + stQWebRsp);
                    }
                    notifyObserver(intent, 1050, false, bundle, MiniAppObserver.class);
                }
            } else {
                if (QLog.isColorLevel()) {
                    QLog.d(TAG, 2, "onReceive. inform MiniAppSetUserSwitchServlet resultcode fail.");
                }
                notifyObserver(intent, 1050, false, bundle, MiniAppObserver.class);
            }
        } catch (Throwable th5) {
    ...
    public void onSend(Intent intent, Packet packet) {
        ...
        byte[] encode = new MiniAppSetUserSwitchRequest(stCommonExt, stringExtra, intExtra).encode(intent, this.index, getTraceId());
        if (encode == null) {
            encode = new byte[4];
        }
        packet.setSSOCommand(MiniAppSetUserSwitchRequest.CMD_STRING);
