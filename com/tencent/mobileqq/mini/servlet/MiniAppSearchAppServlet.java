...
public class MiniAppSearchAppServlet extends MiniAppAbstractServlet {
    public static final String KEY_EXT_INFO = "key_search_app_ext_info";
    public static final String KEY_FROM = "search_app_from";
        ...
        try {
            ...
            if (fromServiceMsg != null) {
                if (fromServiceMsg.isSuccess()) {
                    ...
                    bundle.putParcelable(KEY_SEARCH_APP, fromServiceMsg);
                    notifyObserver(intent, 1072, true, bundle, MiniAppObserver.class);
                } else {
                    bundle.putLong("retCode", fromServiceMsg.getBusinessFailCode());
                    bundle.putString("errMsg", fromServiceMsg.getBusinessFailMsg());
                    notifyObserver(intent, 1072, false, bundle, MiniAppObserver.class);
                }
            } else {
                QLog.e(TAG, 1, "onReceive. inform MiniAppSearchAppServlet response is null.");
            }
        } catch (Throwable th5) {
    ...
    public void onSend(Intent intent, Packet packet) {
        ...
        byte[] encode = new SearchAppRequest(stCommonExt, intent.getStringExtra(KEY_WORD), intent.getIntExtra(KEY_FROM, 1)).encode(intent, intExtra, getTraceId());
        if (encode == null) {
            encode = new byte[4];
        }
        packet.setSSOCommand(SearchAppRequest.CMD_STRING);
