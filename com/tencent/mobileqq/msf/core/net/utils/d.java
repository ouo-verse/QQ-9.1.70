...
public class d implements LayeredSocketFactory {
    ...
    private static final String f249509c = "SNISocketFactory";

    /* renamed from: d, reason: collision with root package name */
    static final HostnameVerifier f249510d;

    /* renamed from: a, reason: collision with root package name */
    private String f249511a;

    /* renamed from: b, reason: collision with root package name */
    SSLCertificateSocketFactory f249512b;

    static {
        IPatchRedirector redirector = PatchRedirectCenter.getRedirector(7051);
        $redirector_ = redirector;
        if (redirector != null && redirector.hasPatch((short) 2)) {
            redirector.redirect((short) 2);
        } else {
            f249510d = new StrictHostnameVerifier();
        }
    }
    ...
    public d(String str) {
        ...
        if (iPatchRedirector != null && iPatchRedirector.hasPatch((short) 1)) {
            ...
            this.f249512b = (SSLCertificateSocketFactory) SSLCertificateSocketFactory.getInsecure(0, null);
            this.f249511a = str;
        }
    }
    ...
    public Socket createSocket(Socket socket, String str, int i3, boolean z16) throws IOException {
        ...
        if (QLog.isColorLevel()) {
            QLog.i(f249509c, 2, "createSocket " + socket.toString() + " host:" + str + " port:" + i3 + " autoClose:" + z16);
        }
        SSLSocket sSLSocket = (SSLSocket) this.f249512b.createSocket(socket, str, i3, z16);
        sSLSocket.setEnabledProtocols(sSLSocket.getSupportedProtocols());
        if (QLog.isColorLevel()) {
            QLog.i(f249509c, 2, "Setting SNI hostname");
        }
        this.f249512b.setHostname(sSLSocket, str);
        SSLSession session = sSLSocket.getSession();
        if (f249510d.verify(str, session)) {
            if (QLog.isColorLevel()) {
                QLog.i(f249509c, 2, "Established " + session.getProtocol() + " connection with " + session.getPeerHost() + " using " + session.getCipherSuite());
            }
            return sSLSocket;
        }
