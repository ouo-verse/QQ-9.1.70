...
public class ThreadAioDownloadPool extends ThreadSmartPool {
    ...
    static {
        IPatchRedirector redirector = PatchRedirectCenter.getRedirector(40956);
        $redirector_ = redirector;
        if (redirector != null && redirector.hasPatch((short) 5)) {
            redirector.redirect((short) 5);
        } else {
            MAX_POOL_SIZE = Math.max(3, ThreadSmartPool.maximumPoolSize);
        }
    }

    ThreadAioDownloadPool(BlockingQueue<Runnable> blockingQueue, PriorityThreadFactory priorityThreadFactory) {
