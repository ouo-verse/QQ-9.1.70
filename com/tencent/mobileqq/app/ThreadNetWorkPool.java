...
public class ThreadNetWorkPool extends ThreadSmartPool {
    static IPatchRedirector $redirector_ = null;
    private static final int BLOCKING_QUEUE_SIZE;
    private static final int CORE_POOL_SIZE = 5;
    private static final int MAX_POOL_SIZE;
    public static final String TAG = "thread_sp_net_";

    static {
        int i3;
        IPatchRedirector redirector = PatchRedirectCenter.getRedirector(41099);
        $redirector_ = redirector;
        if (redirector != null && redirector.hasPatch((short) 5)) {
            redirector.redirect((short) 5);
            return;
        }
        MAX_POOL_SIZE = Math.max(5, ThreadSmartPool.maximumPoolSize) * 2;
    ...
    ThreadNetWorkPool(BlockingQueue<Runnable> blockingQueue, PriorityThreadFactory priorityThreadFactory) {
    ...
    public static ThreadSmartPool createThreadPool() {
        return new ThreadNetWorkPool(new LinkedBlockingQueue(BLOCKING_QUEUE_SIZE), new PriorityThreadFactory(TAG, 2));
    }
