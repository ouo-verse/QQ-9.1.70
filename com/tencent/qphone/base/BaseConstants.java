package com.tencent.qphone.base;

import com.tencent.mobileqq.msf.core.x.c;

/* compiled from: P */
/* loaded from: classes22.dex */
public class BaseConstants {
    public static final String ACTION_MSF_NOTIFY_REGISTER_SERVICE = "com.tencent.mobileqq.msf.ACTION_MSF_NOTIFY_REGISTER_SERVICE";
    public static final String ACTION_PROCESS_EXIT = "com.tencent.process.exit";
    public static final String ATTRIBUTE_ALARM_ELAPSED_TIEM = "_attr_alarm_elapsed_time";
    public static final String ATTRIBUTE_DISABLE_MERGE = "_attr_disable_merge";
    public static final String ATTRIBUTE_IP_FAMILY = "_attr_ip_family";
    public static final String ATTRIBUTE_ISP = "_attr_isp";
    public static final String ATTRIBUTE_IS_WIFI = "_attr_is_wifi";
    public static final String ATTRIBUTE_KEY_APP_CLONE = "key_app_clone";
    public static final String ATTRIBUTE_KEY_EXTRA_DEVICE_INFO = "extraDeviceInfo";
    public static final String ATTRIBUTE_KEY_GATEWAY_IP = "key_msf_gateway_ip";
    public static final String ATTRIBUTE_KEY_GET_WEAK_NET = "attr_key_get_weak_new";
    public static final String ATTRIBUTE_KEY_IS_BAD_NETWORK = "attr_key_is_bad_network";
    public static final String ATTRIBUTE_KEY_IS_MSF_CONNECT = "attr_key_is_msf_connect";
    public static final String ATTRIBUTE_KEY_IS_SECURITY_SIGN = "attr_key_is_security_sign";
    public static final String ATTRIBUTE_KEY_LOGREPORT_COLOR_LEVEL = "attr_key_logreport_color_level";
    public static final String ATTRIBUTE_KEY_LOGREPORT_ENDTIME = "attr_key_logreport_endtime";
    public static final String ATTRIBUTE_KEY_LOGREPORT_PSKEY = "attr_key_logreport_pskey";
    public static final String ATTRIBUTE_KEY_LOGREPORT_QIMEI36 = "attr_key_logreport_qimei36";
    public static final String ATTRIBUTE_KEY_LOGREPORT_STARTTIME = "attr_key_logreport_starttime";
    public static final String ATTRIBUTE_KEY_LOGREPORT_TASK_ID = "attr_key_logreport_task_id";
    public static final String ATTRIBUTE_KEY_LOGREPORT_WITHOUTLOGIN = "attr_key_logreport_withoutLogin";
    public static final String ATTRIBUTE_KEY_MSF_CORE_STATE_PARAM_MAP = "attr_msf_core_state_param_map";
    public static final String ATTRIBUTE_KEY_MSF_TOGGLE = "attr_key_msf_toggle";
    public static final String ATTRIBUTE_KEY_MSF_V20_TOGGLE = "key_msf_v20_toggle";
    public static final String ATTRIBUTE_KEY_NT_KERNEL_FLAG = "attr_key_nt_kernel_flag";
    public static final String ATTRIBUTE_KEY_PB_DATA = "pbData";
    public static final String ATTRIBUTE_KEY_RECV_TIME = "attr_key_recv_time";
    public static final String ATTRIBUTE_KEY_RECV_WAY = "attr_key_recv_way";
    public static final String ATTRIBUTE_KEY_SECURITY_CALLBACK_ID = "attr_key_security_callback_id";
    public static final String ATTRIBUTE_KEY_SECURITY_INIT_COST_TIME = "attr_key_is_security_init_cost_time";
    public static final String ATTRIBUTE_KEY_SECURITY_SIGN_COST_TIME = "attr_key_is_security_sign_cost_time";
    public static final String ATTRIBUTE_KEY_SECURITY_SIGN_FLAG = "attr_key_security_sign_flag";
    public static final String ATTRIBUTE_KEY_SEND_TIME = "attr_key_send_time";
    public static final String ATTRIBUTE_KEY_SERVER_TIME = "attr_key_server_time";
    public static final String ATTRIBUTE_KEY_WRITE_SOCKET_TIME = "attr_key_write_socket_time";
    public static final String ATTRIBUTE_MSG_HAS_RESEND_BY_10008 = "_attr_msg_has_resend_by_10008";
    public static final String ATTRIBUTE_MSG_HAS_RESEND_BY_10119 = "_attr_msg_has_resend_by_10119";
    public static final String ATTRIBUTE_NEED_REMIND_SLOW_NETWORK = "remind_slown_network";
    public static final String ATTRIBUTE_REGPRXY_RANDOM_CODE = "_attr_regprxy_random_code";
    public static final String ATTRIBUTE_REGPRXY_SOCKET_HASHCODE = "_attr_regprxy_socket_hashcode";
    public static final String ATTRIBUTE_RESP_GRAY_CHECK = "resp_gray_check";
    public static final String ATTRIBUTE_RESP_LOGIN_LHSIG = "resp_login_lhsig";
    public static final String ATTRIBUTE_RESP_LOGIN_RET = "resp_logini_ret";
    public static final String ATTRIBUTE_RESP_LOGIN_URL = "resp_login_url";
    public static final String ATTRIBUTE_SAMEDEVICE = "_attr_sameDevice";
    public static final String ATTRIBUTE_SEND_BY_MAIN = "attr_send_by_main";
    public static final String ATTRIBUTE_SEND_BY_QUICKHTTP = "_attr_send_by_quickHttp";
    public static final String ATTRIBUTE_SSO_IP = "_attr_sso_ip";
    public static final String ATTRIBUTE_USE_UDP = "_attr_use_udp";
    public static final String ATTRIBUT_REQUEST_ISCONN = "_attr_req_isconn";
    public static final String ATTRIBUT_REQUEST_NETSTATE = "_attr_req_netstate";
    public static final String ATTRIBUT_REQUEST_SENDTIME = "_attr_req_send_time";
    public static final String ATTRIBUT_REQUEST_SOCKET_CONN_TIME = "_attr_req_socket_conn_time";
    public static final String ATTRIBUT_TIMEOUT_ISCONN = "_attr_timeout_isconn";
    public static final String ATTRIBUT_TIMEOUT_NETSTATE = "_attr_timeout_netstate";
    public static final String ATTRIBUT_TIMEOUT_SOCKET_CONN_TIME = "_attr_timeout_socket_conn_time";
    public static final String ATTR_KET_BUSINESS_SQE = "key_business_seq";
    public static final String ATTR_KET_BUSINESS_TYPE = "businessType";
    public static final String ATTR_KET_ERR_MSG = "errMsg";
    public static final String ATTR_KET_EXTRA_MAP = "extraMap";
    public static final String ATTR_KET_PHONE_TOKEN = "phoneToken";
    public static final String ATTR_KET_RET = "ret";
    public static final String ATTR_KET_SIF_SESSION = "sigSession";
    public static final String ATTR_KET_SSO_SEQ = "key_sso_seq";
    public static final String ATTR_KET_TO_SERVICE_MSG = "key_to_service_msg";
    public static final String ATTR_KET_UIN = "uin";
    public static final String ATTR_KET_USER_SIG_INFO = "userSigInfo";
    public static final String ATTR_KEY_SEC_DISPATCH_EVENT_CMD = "attr_key_sec_dispatch_event_cmd";
    public static final String ATTR_KEY_SEC_DISPATCH_EVENT_DATA = "attr_key_sec_dispatch_event_data";
    public static final String ATTR_KEY_SEC_DISPATCH_EVENT_PARAM = "attr_key_sec_dispatch_event_param";
    public static final String ATTR_KEY_SEC_DISPATCH_EVENT_RET = "attr_key_sec_dispatch_event_ret";
    public static final String Attribute_DEVICEGUID = "_attr_deviceGUID";
    public static final String Attribute_RESP_APP_TIMEOUT = "_attr_app_timeout";
    public static final String Attribute_SERVERTIME = "_attr_server";
    public static final String Attribute_SOCKET_CONNSTATE = "_attr_socket_connstate";
    public static final String Attribute_TAG_CURRENTDAYDATA = "__base_tag_curentDay";
    public static final String Attribute_TAG_CURRENTDAYDATA_WiFi = "__base_tag_curentDay_WiFi";
    public static final String Attribute_TAG_CURRENTMONTHDATA = "__base_tag_curentMonth";
    public static final String Attribute_TAG_CURRENTMONTHDATA_WiFi = "__base_tag_curentMonth_WiFi";
    public static final String Attribute_TAG_ISAPPMSG = "__base_tag_isAppMsg";
    public static final String Attribute_TAG_LOGSTR = "_tag_LOGSTR";
    public static final String Attribute_TAG_SENDSUCC = "__attribute_tag_sendsucc";
    public static final String Attribute_TAG_SENDTIMEOUT = "__attribute_tag_sendtimeout";
    public static final String Attribute_TAG_SERVERTIME = "__attribute_tag_servertime";
    public static final String Attribute_TAG_SERVERTIMEDIFF = "__attribute_tag_servertimediff";
    public static final String Attribute_TAG_SIGFAILCODE = "__base_tag_sigFailCode";
    public static final String Attribute_TAG_SIGSESSION = "__base_tag_sigSession";
    public static final String Attribute_TAG_SOCKET_ADDRESS = "_tag_socket";
    public static final String Attribute_TAG_SOCKET_CONNERROR = "_tag_socket_connerror";
    public static final String BROADCAST_ACTION_USERSYNC = "com.tencent.mobileqq.usersync";
    public static final String BROADCAST_ACTION_WEAK_NET_CHANGE = "com.tencent.mobileqq.msf.bd.weaknetchange";
    public static final String BROADCAST_USERSYNC_ACTION = "action";
    public static final String BROADCAST_USERSYNC_ENTER = "enter";
    public static final String BROADCAST_USERSYNC_EXIT = "exit";
    public static final String BROADCAST_USERSYNC_UIN = "uin";
    public static final int BUFLAG_B1 = 16;
    public static final String CMD_APPUSEETLOGIN = "cmd_appUseWtLogin";
    public static final String CMD_APP_CLEARDATACOUNT = "App_CLearDataCount";
    public static final String CMD_APP_CLONE = "cmd_appClone";
    public static final String CMD_APP_DATAINCERMENT = "App_DataIncerment";
    public static final String CMD_APP_GETDATACOUNT = "App_GetDataCount";
    public static final String CMD_APP_REPORTLOG = "App_ReportLog";
    public static final String CMD_AV_SHARP = "SharpSvr.";
    public static final String CMD_BEFORE_MSG_SYNC_CONFIG = "cmd_msg_sync_config";
    public static final String CMD_CHANGETOKEN = "login.chgTok";
    public static final String CMD_CHANGETOKEN_A2D2 = "login.chgTok_A2D2";
    public static final String CMD_CHANGETOKEN_DA2 = "login.chgTok_DA2";
    public static final String CMD_CHANGETOKEN_SKEY = "login.chgTok_SKEY";
    public static final String CMD_CHANGETOKEN_WEBVIEW_KEY = "login.chgTok_WEBVIEW_KEY";
    public static final String CMD_CHECK_MSF_CONERRO = "Check_msf_conErro";
    public static final String CMD_CONFIG_APPCHECKUPDATE = "ConfigService.ClientReq";
    public static final String CMD_CONNALLFAILED = "cmd_connAllFailed";
    public static final String CMD_CONNAUTHSVR_GETAPPINFO = "ConnAuthSvr.get_app_info_emp";
    public static final String CMD_CONNAUTHSVR_GETAUTHAPI = "ConnAuthSvr.sdk_auth_api_emp";
    public static final String CMD_CONNAUTHSVR_GETAUTHAPILIST = "ConnAuthSvr.get_auth_api_list_emp";
    public static final String CMD_CONNCLOSED = "cmd_connClosed";
    public static final String CMD_CONNOPENED = "cmd_connOpened";
    public static final String CMD_CONNWEAKNET = "cmd_connWeakNet";
    public static final String CMD_CONNWEAKNET_NEW = "cmd_connWeakNet_New";
    public static final String CMD_CONN_PROBE = "StatSvc.MsfConnProbe";
    public static final String CMD_DELLOGINEDACCOUNT = "cmd_DelLoginedAccount";
    public static final String CMD_FORCEOFFLINERESP = "StatSvc.RspMSFForceOffline";
    public static final String CMD_FORCE_LOGOUT = "MessageSvc.PushForceOffline";
    public static final String CMD_GETCONFIG_NEW = "ResourceConfig.ClientReq";
    public static final String CMD_GETCONFIG_OLD = "ConfigService.ClientReq";
    public static final String CMD_GETGATEWAYIP = "cmd_getGatewayIp";
    public static final String CMD_GETKEY = "MSF.getKey";
    public static final String CMD_GETPICSERLIST = "ConfigService.GetPictureServerReq";
    public static final String CMD_GETPLUGINCONFIG = "cmd_getPluginConfig";
    public static final String CMD_GETSERVERCONFIG = "cmd_getServerConfig";
    public static final String CMD_GETTIME = "login.time";
    public static final String CMD_GETTIMENEW = "Client.CorrectTime";
    public static final String CMD_GET_SID = "login.getSid";
    public static final String CMD_GET_WEAK_NET = "cmd_get_weakNet";
    public static final String CMD_GRAY_APPLY = "VCService.Apply";
    public static final String CMD_GRAY_CHECK = "GrayUinPro.Check";
    public static final String CMD_HEARTBEATALIVE = "Heartbeat.Alive";
    public static final String CMD_HEARTBEATPING = "Heartbeat.Ping";
    public static final String CMD_IMG_STORE_GROUP_PIC_UP = "ImgStore.GroupPicUp";
    public static final String CMD_INVALIDSIGN = "cmd_InvalidSign";
    public static final String CMD_KEEP_PROCESS_ALIVE = "cmd_keep_process_alive";
    public static final String CMD_LISTEN_MSG_SYNC_END = "cmd_listen_msg_sync_end";
    public static final String CMD_LISTEN_ONLINE_REGISTER = "cmd_listen_online_register";
    public static final String CMD_LOGIN_AUTH = "login.auth";
    public static final String CMD_LOGIN_CHANGEUIN_AUTH = "login.chguin";
    public static final String CMD_LOGIN_REGISTR_COMMITMOBILE = "login.register.CommitMobile";
    public static final String CMD_LOGIN_REGISTR_COMMITPASS = "login.register.CommitPass";
    public static final String CMD_LOGIN_REGISTR_COMMITSMS = "login.register.CommitSms";
    public static final String CMD_LOGIN_REGISTR_QUERYSMS = "login.register.QuerySms";
    public static final String CMD_LOGIN_REGISTR_RESENDSMS = "login.register.ResendSms";
    public static final String CMD_LONG_CONN_OFF_PIC_UP = "LongConn.OffPicUp";
    public static final String CMD_MSFCOMMAND = "msfCommand";
    public static final String CMD_MSF_CONN_PROBE_ACK = "trpc.qq_new_tech.status_svc.StatusService.SsoAckMsfProbe";
    public static final String CMD_MSF_CORE_STATE = "cmd_msfCore_state";
    public static final String CMD_MSF_NOTIFYRESP = "baseSdk.Msf.NotifyResp";
    public static final String CMD_MSF_PUSHRESP = "baseSdk.Msf.PushResp";
    public static final String CMD_MSF_REGISTERAPP = "baseSdk.Mul.PingCallback";
    public static final String CMD_MSF_TEST_CLOSE_CONN = "cmd_msf_test_close_conn";
    public static final String CMD_MSG_UNREAD_MSG_SEQ = "PbMessageSvc.PbUnReadMsgSeq";
    public static final String CMD_NETEXCEPTION = "Msf.NetException";
    public static final String CMD_NETNEEDSIGNON = "cmd_netNeedSignon";
    public static final String CMD_NETWORKTRAFFICDEBUGINFO = "App_NerworkTrafficDebugInfo";
    public static final String CMD_NEW_REGPRXYSVC_INFOSYNC = "RegPrxySvc.infoSync";
    public static final String CMD_NEW_REGPRXYSVC_PBSYNCMSG = "RegPrxySvc.PbSyncMsg";
    public static final String CMD_NOTIFY_QQWIFI_AVAILABLE = "qqwifi.notifyAvail";
    public static final String CMD_NOTIFY_WEAK_NET_SWITCH = "notifyWeakNetSwitch";
    public static final String CMD_NT_PUSH_REGISTER = "trpc.qq_new_tech.status_svc.StatusService.Register";
    public static final String CMD_NT_PUSH_UNREGISTER = "trpc.qq_new_tech.status_svc.StatusService.UnRegister";
    public static final String CMD_NT_REGPRXSYNC_BATCH_GET_MSG = "trpc.msg.register_proxy.RegisterProxy.SsoBatchGetC2cMsg";
    public static final String CMD_NT_REGPRXSYNC_GROUP_MSG = "trpc.msg.register_proxy.RegisterProxy.SsoSyncGroupMsg";
    public static final String CMD_NT_REGPRXYSVC_INFOSYNC = "trpc.msg.register_proxy.RegisterProxy.SsoInfoSync";
    public static final String CMD_NT_REGPRXYSVC_PUSH_C2C_MSG = "trpc.msg.register_proxy.RegisterProxy.C2cMsgPush";
    public static final String CMD_NT_REGPRXYSVC_PUSH_GROUP_MSG = "trpc.msg.register_proxy.RegisterProxy.GroupMsgPush";
    public static final String CMD_NT_REGPRXYSVC_PUSH_INFOSYNC = "trpc.msg.register_proxy.RegisterProxy.InfoSyncPush";
    public static final String CMD_NT_REGPRXYSVC_PUSH_PARAMS = "trpc.msg.register_proxy.RegisterProxy.PushParams";
    public static final String CMD_NT_REGPRXYSVC_SSO_GET_C2C_MSG = "trpc.msg.register_proxy.RegisterProxy.SsoGetC2cMsg";
    public static final String CMD_NT_REGPRXYSVC_SSO_GET_GROUP_MSG = "trpc.msg.register_proxy.RegisterProxy.SsoGetGroupMsg";
    public static final String CMD_NT_REGPRXYSVC_SSO_GET_GROUP_SHARE_MSG = "trpc.msg.register_proxy.RegisterProxy.SsoGetGroupShareMsg";
    public static final String CMD_NT_REGPRXYSVC_SSO_GET_ROAM_MSG = "trpc.msg.register_proxy.RegisterProxy.SsoGetRoamMsg";
    public static final String CMD_NT_SSO_GET_PEER_SEQ = "trpc.msg.msg_svc.MsgService.SsoGetPeerSeq";
    public static final String CMD_NT_SSO_HEART_BEAT = "trpc.qq_new_tech.status_svc.StatusService.SsoHeartBeat";
    public static final String CMD_OFFLINEPUSH = "StatSvc.ReqMSFOffline";
    public static final String CMD_OFFLINE_PUSH = "trpc.qq_new_tech.status_svc.StatusService.KickNT";
    public static final String CMD_ONLINESTATUSCHANGED = "Msf.OnlineStatusChanged";
    public static final String CMD_OPENCONN = "cmd_openConn";
    public static final String CMD_OVER_LOAD_PUSH_NOTIFY = "OverLoadPush.notify";
    public static final String CMD_PBPUSHOFFMSG = "OnlinePush.PbPushOffMsg";
    public static final String CMD_PB_PUSH_REQ = "trpc.qqaccess.dispatch.DispatchService.Push";
    public static final String CMD_PB_PUSH_RESP = "trpc.qqaccess.dispatch.DispatchService.PushAck";
    public static final String CMD_PUSHCHANGETOKEN = "wtlogin.PushChangeToken";
    public static final String CMD_PUSHREADED = "OnlinePush.PushOffReadedNotify";
    public static final String CMD_PUSHREQ = "ConfigPushSvc.PushReq";
    public static final String CMD_PUSHRESP = "ConfigPushSvc.PushResp";
    public static final String CMD_PUSHSETCONFIG = "cmd_pushSetConfig";
    public static final String CMD_PUSH_DOMAIN = "ConfigPushSvc.PushDomain";
    public static final String CMD_PUSH_REGISTERPROXY = "push.proxyRegister";
    public static final String CMD_PUSH_REGISTER_SWITCH = "cmd_push_register_switch";
    public static final String CMD_PUSH_SID_EXPIRED = "OnlinePush.SidTicketExpired";
    public static final String CMD_PUSH_UNREGISTERPROXY = "push.proxyUnRegister";
    public static final String CMD_QUALITYTEST = "QualityTest.PushList";
    public static final String CMD_RECVFIRSTRESP = "cmd_recvFirstResp";
    public static final String CMD_REFRESH_TICKETS = "cmd_refreshTickets";
    public static final String CMD_REGISTERMSFSERVICE = "cmd_RegisterMsfService";
    public static final String CMD_REGISTER_CMDCALLBACKER = "push.cmdRegister";
    public static final String CMD_REGISTER_NOTIFY = "notify.register";
    public static final String CMD_REGISTER_PUSH = "push.register";
    public static final String CMD_REGPRXYSVC_GETOFFMSG = "RegPrxySvc.getOffMsg";
    public static final String CMD_REGPRXYSVC_INFOLOGIN = "RegPrxySvc.infoLogin";
    public static final String CMD_REGPRXYSVC_NOTICE_END = "RegPrxySvc.NoticeEnd";
    public static final String CMD_REPORTRDM = "App_reportRDM";
    public static final String CMD_REPORTSTAT = "CliLogSvc.UploadReq";
    public static final String CMD_REQUEST_CONFIG = "ConfigurationService.ReqGetConfig";
    public static final String CMD_REQ_CHECKSIGNATURE = "PhSigLcId.Check";
    public static final String CMD_RESET_CMDCALLBACKER = "push.resetCmds";
    public static final String CMD_RESPPUSH = "OnlinePush.RespPush";
    public static final String CMD_RE_NOTIFY_CONN_OPENED = "cmd_re_notify_conn_open";
    public static final String CMD_SA = "login.";
    public static final String CMD_SA_WT = "wtlogin.";
    public static final String CMD_SDK_INTERNAL = "sdkInternalCmd";
    public static final String CMD_SEC_DISPATCH_EVENT = "cmd_sec_dispatch_event";
    public static final String CMD_SETMSFCONNSTATUS = "cmd_getSetMsfConnStatus";
    public static final String CMD_SETRESUME = "Msf.SetResume";
    public static final String CMD_SETSUSPEND = "Msf.SetSuspend";
    public static final String CMD_SSOHELLOPUSH = "SSO.HelloPush";
    public static final String CMD_SSO_LOGIN_MERGE = "SSO.LoginMerge";
    public static final String CMD_START_CONNECT = "cmd_startConnect";
    public static final String CMD_STATUS_SVC_MSF_HELLO = "trpc.qq_new_tech.status_svc.StatusService.SsoMsfHello";
    public static final String CMD_SUFFIX_REFRESH_VERIFY_CODE = ".__refresh";
    public static final String CMD_SUFFIX_SEND_VERIFY_CODE = ".__verify";
    public static final String CMD_SYNC_SYNCUSER = "cmd_sync_syncuser";
    public static final String CMD_SYNC_SYNCUSER_DEL = "cmd_sync_syncuser_del";
    public static final String CMD_SYNC_SYNCUSER_DEL_REPORT = "cmd_sync_syncuser_del_report";
    public static final String CMD_SYNC_SYNCUSER_REPORT = "cmd_sync_syncuser_report";
    public static final String CMD_SYNC_SYNCUSER_SERVICE = "cmd_sync_syncuser_service";
    public static final String CMD_UNREGISTERMSFSERVICE = "cmd_UnRegisterMsfService";
    public static final String CMD_UNREGISTER_NOTIFY = "notify.unRegister";
    public static final String CMD_UNREGISTER_PUSH = "push.unRegister";
    public static final String CMD_UPDATE_COMPLEX_CONNECT_CONFIG = "cmd_update_complex_connect_config";
    public static final String CMD_UPDATE_MANAGER_CONFIG = "cmd_update_manager_config";
    public static final String CMD_UPDATE_MMKV_CONFIG = "cmd_update_mmkv_config";
    public static final String CMD_UPDATE_MSF_COMMON_CONFIG = "cmd_update_msf_common_config";
    public static final String CMD_UPDATE_MSF_WEAK_NET_CONFIG = "cmd_update_msf_weaknet_config";
    public static final String CMD_UPDATE_TOGGLE_CONFIG = "cmd_update_toggle_config";
    public static final String CMD_USERENTER = "cmd_userEnter";
    public static final String CMD_USEREXIT = "cmd_userExit";
    public static final String CMD_VERIFY_PASSWD = "login.verifyPasswd";
    public static final String CMD_VERIFY_PASSWD_IMAGE = "login.verifyPasswdImage";
    public static final String CMD_VERIFY_PASSWD_REFRESH_IMAGE = "login.verifyPasswdRefreshImage";
    public static final String CMD_WAKE_FROM_DS = "cmd_wake_from_deep_sleep";
    public static final String CMD_WIRELESSMEIBAOREQ = "PwdProtect.DataUpload";
    public static final String CMD_WIRELESSPSWREQ = "KsidTrans.Trans";
    public static final String CMD_WT_LOGIN_ADDCONTACTS = "account.RequestVerifyWTLogin_emp";
    public static final String CMD_WT_LOGIN_AUTH = "wtlogin.login";
    public static final String CMD_WT_LOGIN_EXCHANGE = "wtlogin.exchange";
    public static final String CMD_WT_LOGIN_NAME2UIN = "wtlogin.name2uin";
    public static final String CMD_WT_LOGIN_REGACCOUNT = "wtlogin.register";
    public static final String CMD_WT_LOGIN_REQUESTREBINDMBL = "account.RequestReBindMblWTLogin_emp";
    public static final int CODE_APPTIMEOUT = 1013;
    public static final int CODE_APP_CLONE = 3003;
    public static final int CODE_BASESERVICENOTFOUND = 1006;
    public static final int CODE_CHANGEUINFAILED = 2010;
    public static final int CODE_CLIENTTIMEERROR = 1012;
    public static final int CODE_EXCEPITON = 1005;
    public static final int CODE_FAIL = 1001;
    public static final int CODE_FREQUENCYERROR = 1010;
    public static final int CODE_INVALIDREQUEST = 1007;
    public static final int CODE_INVALIDSIGN = 2014;
    public static final int CODE_LOAD_SERVICE_FAIL = 1004;
    public static final int CODE_NEED_IMAGE_VERIFY = 2020;
    public static final int CODE_NO_LOGIN = 2001;
    public static final int CODE_NO_SERVICE_FOUND = 1003;
    public static final int CODE_OK = 1000;
    public static final int CODE_OVERLOAD_PROTECTION = -20009;
    public static final int CODE_PASS_ERROR = 2005;
    public static final int CODE_PROXYUNREGISTERFAILED = 1011;
    public static final int CODE_QUEUEFULL = 1008;
    public static final int CODE_REQ_TOO_LARRG = 2019;
    public static final int CODE_RESENDMSG = 2901;
    public static final int CODE_SDKSUSPENDED = 2009;
    public static final int CODE_SENDERROR = 1009;
    public static final int CODE_SEND_WITH_NO_AUTH_ERROR = 2902;
    public static final int CODE_SERVER_RETURN_ERROR = 2900;
    public static final int CODE_SSO_Invalid_Cookie = 2015;
    public static final int CODE_SSO_KICKED = 2013;
    public static final int CODE_SSO_KICKEDANDCLEARTOKEN = 2012;
    public static final int CODE_SSO_TIPS = 2011;
    public static final int CODE_STANDBYMODE_FORCE_STOP = 2018;
    public static final int CODE_SUBSERVICE_START = 2004;
    public static final int CODE_TEST_IPC_PACKAGE_LENGTH = -2;
    public static final int CODE_TIMEOUT = 1002;
    public static final int CODE_USERCANCEL = 2006;
    public static final int CODE_USER_GRAYFAIL = 2008;
    public static final int CODE_USER_REGISTER = 2007;
    public static final int CODE_VERIFY_CODE = 2002;
    public static final int CODE_VERIFY_CODETIMEOUT = 2003;
    public static final int CODE_VERIFY_DEV_LOCK_SMS = 2016;
    public static final int CODE_VERIFY_DEV_LOCK_SMS_TIMEOUT = 2017;
    public static final int CODE_WAITRESPTIMEOUT = 1014;
    public static final int CODE_WTWRROR = 3001;
    public static final int CODE_WT_EXCEPTION = 3002;
    public static final String COMMAND_RequestPullUnreadMsgCount = "MessageSvc.PullUnreadMsgCount";
    public static final int CONFIG_TYPE_APP = 2;
    public static final int CONFIG_TYPE_COMMON = 0;
    public static final int CONFIG_TYPE_USER = 1;
    public static final long DEFAULT_MSG_TIMEOUT = 30000;
    public static final long DEFAULT_QUICK_HEARTBEAT_RECONN_INTERVAL = 120000;
    public static final long DEFAULT_QUICK_HEARTBEAT_TIMEOUT = 10000;
    public static final int DEFAULT_THR_DEPTH = 10;
    public static final String DEFAULT_UNKNOWN_BSSID = "UNKNOWN_WIFI_BSSID";
    public static final String DEFAULT_UNKNOWN_SSID = "UNKNOWN_WIFI_SSID";
    public static final String DEFAULT_UNKNOWN_XG_NET_IDENTIFIER = "UNKNOWN_XG_NET_IDENTIFIER";
    public static final String DEFAULT_UNKNOWN_XG_NET_NAME = "UNKNOWN_XG_NET_NAME";
    public static final String DELTA_TIME_SERVER = "__delta_time_server";
    public static final String EVENT_THREAD_ERROR = "event_thread_error";
    public static final String EXTRA_NOTIFYID = "notifyId";
    public static final String EXTRA_PUSHID = "pushId";
    public static final String EXTRA_PUSHMSG = "pushMsgs";
    public static final String FLOW_ALL_KEY = "param_Flow";
    public static final String FLOW_WIFI_CHAT_KEY = "param_WIFIChatFlow";
    public static final String FLOW_WIFI_KEY = "param_WIFIFlow";
    public static final String FLOW_WIFI_RDM_KEY = "param_WIFIRdmFlow";
    public static final String FLOW_XG_CHAT_KEY = "param_XGChatFlow";
    public static final String FLOW_XG_KEY = "param_XGFlow";
    public static final String FLOW_XG_RDM_KEY = "param_XGRdmFlow";
    public static final String INTENT_KEY_IS_WEAK_NET = "is_weak_net";
    public static final String INTENT_KEY_WEAK_NET_STATUS_CHANGE_REASON = "weak_net_status_change_reason";
    public static final int KEY_ACTION_BNBC = 5;
    public static final int KEY_ACTION_CONN_STATE = 1;
    public static final int KEY_ACTION_HB_COST = 3;
    public static final int KEY_ACTION_PING_COST = 2;
    public static final int KEY_ACTION_REASON = 6;
    public static final int KEY_ACTION_RTT = 4;
    public static final int KEY_ACTION_RTT_WINDOW = 7;
    public static final String KEY_BEFORE_MSG_SYNC_CONFIG_ALL = "key_before_msg_sync_config";
    public static final String KEY_BNBC_STRATEGY = "BnBcStrategy";
    public static final String KEY_CLIENT_CONN_SEQ = "client_conn_seq";
    public static final String KEY_COMPLEX_CONNECT_CONFIG_ALL = "key_complex_connect_config_all";
    public static final String KEY_CONFIG_CONTENT = "key_config_content";
    public static final String KEY_CONFIG_MANAGER_ID = "key_config_manager_id";
    public static final String KEY_CONN_COST = "connCost";
    public static final String KEY_COUNT_BC = "countBC";
    public static final String KEY_COUNT_BN = "countBN";
    public static final String KEY_ERROR_THR = "key_error_thr";
    public static final String KEY_ERROR_TIME = "key_error_time";
    public static final String KEY_IS_BLOCK_PUSH_REGISTER = "key_is_block_push_register";
    public static final String KEY_IS_NEW_WEAK_NET_ENABLE = "isNewWeakNetEnable";
    public static final String KEY_IS_QUICK_SEND_CHECK = "isQuickSendCheck";
    public static final String KEY_IS_SCREEN_OFF_CHECK = "isScreenOffCheck";
    public static final String KEY_IS_USE_NEW_WEAK_NET_DETECTOR = "isUseNewWeakNetDetector";
    public static final String KEY_IS_USE_SSO_ROUTE_COST = "isUseSSORouteCost";
    public static final String KEY_IS_WEAK_NET_ENABLE = "isWeakNetEnable";
    public static final String KEY_MAX_HEART_BEAT_COST = "maxHeartBeatCost";
    public static final String KEY_MIN_HEART_BEAT_COST = "minHeartBeatCost";
    public static final String KEY_MSF_COMMON_CONFIG_STRING = "key_msf_common_config_string";
    public static final String KEY_MSF_CORE_CONN_STATE = "key_msfCore_connState";
    public static final String KEY_MSF_CORE_PACKET_SIZE = "key_msfCore_packet_size";
    public static final String KEY_MSF_CORE_RTT = "key_msfCore_rtt";
    public static final String KEY_MSF_MMKV_CONFIG = "key_msf_mmkv_config";
    public static final String KEY_MSF_RET_CODE = "msf_ret_code";
    public static final String KEY_MSF_WEAK_NET_CONFIG_ALL = "key_msf_weakNet_config_all";
    public static final String KEY_PING_COST = "pingCost";
    public static final String KEY_RTT_LOW_BOUND = "RttLowBound";
    public static final String KEY_RTT_LOW_BOUND_FACTOR = "RttLowBoundFactor";
    public static final String KEY_RTT_UP_BOUND = "RttUpBound";
    public static final String KEY_RTT_UP_BOUND_FACTOR = "RttUpBoundFactor";
    public static final String KEY_SEC_EVENT = "key_sec_event";
    public static final String KEY_SSO_RET_CODE = "sso_ret_code";
    public static final String KEY_THREAD_NAME = "key_thread_name";
    public static final String KEY_THREAD_PID = "key_thread_pid";
    public static final String KEY_TIMEOUT_BNBC = "timeoutBNBC";
    public static final String KEY_TIME_WAIT_QUICK_SEND = "waitTimeQuickSend";
    public static final String KEY_WEAK_NET_CONFIG = "weakNetConfig";
    public static final String KEY_WEAK_NET_DEBUG_BC = "key_weak_net_debug_bc";
    public static final String KEY_WEAK_NET_DEBUG_BN = "key_weak_net_debug_bn";
    public static final String KEY_WEAK_NET_DEBUG_CONN_COST = "key_weak_net_debug_conn_cost";
    public static final String KEY_WEAK_NET_DEBUG_CONN_STATE = "key_weak_net_debug_conn_state";
    public static final String KEY_WEAK_NET_DEBUG_DOWN_BOUND = "key_weak_net_down_bound";
    public static final String KEY_WEAK_NET_DEBUG_FLOAT_SWITCH = "weak_net_debug_float_switch";
    public static final String KEY_WEAK_NET_DEBUG_HB_COST = "key_weak_net_hb_cost";
    public static final String KEY_WEAK_NET_DEBUG_PING_COST = "key_weak_net_debug_ping_cost";
    public static final String KEY_WEAK_NET_DEBUG_REASON = "key_weak_net_debug_reason";
    public static final String KEY_WEAK_NET_DEBUG_RTT = "key_weak_net_debug_rtt";
    public static final String KEY_WEAK_NET_DEBUG_TRTT = "key_weak_net_trtt";
    public static final String KEY_WEAK_NET_DEBUG_UP_BOUND = "key_weak_net_up_bound";
    public static final String KEY_WEAK_NET_PARAM_JSON = "key_weak_net_param_json";
    public static final String KEY_WEAK_NET_STRATEGY = "weakNetStrategy";
    public static final String KEY_WIFI_SIGNAL = "wifiSignal";
    public static final int LANG_ID_SIMPLIFIED_CHINESE = 2052;
    public static int MAXSENDCOUNT_INMINUTE = 150;
    public static final String MINI_SDK = "";
    public static final String MSF_MMKV_FILE = "msf_mmkv_file";
    public static final int MSF_UIN_TYPE_DEFAULT = -1;
    public static final int MSF_UIN_TYPE_QQ = 0;
    public static final String MSF_WEAK_NET_SP_NAME = "msf_weak_net_sp_name";
    public static final String NETEXCEPTION_ATTRIBUTE_EXCEPTYPE = "NetExceptionType";
    public static final int NETEXCEPTION_TYPE_MESSAGETIMEOUT = 1;
    public static final int NETINFO_MOBILE = 1;
    public static final int NETINFO_NEEDAUTHWIFI = 3;
    public static final int NETINFO_NONE = 0;
    public static final int NETINFO_WIFI = 2;
    public static final int NETINFO_WIFIANDMOBILE = 3;
    public static final long OPEN_SDK_BUFLAG_B1 = 1600001540;
    public static String RDM_NoChangeFailCode = "rdm_NCF";
    public static final int REGUIN_CAPTCHA = 4;
    public static final int REGUIN_FACE = 59;
    public static final int REGUIN_GUARANTEE = 8;
    public static final int REGUIN_IFRAME = 9;
    public static final int REGUIN_JUMPURL = 3;
    public static final int REGUIN_QUERYSMS = 4;
    public static final int REGUIN_REJECT = 7;
    public static final int REGUIN_RESENDSMSFAST = 5;
    public static final int REGUIN_SUCC = 0;
    public static final int REGUIN_UPSMS = 2;
    public static final int REGUIN_WAITSMS = 1;
    public static final int RET_REG_PHONE_TOKEN = 61;
    public static final int RET_USER_CANCEL = -20160326;
    public static final int ROUTING_ID = 62;
    public static final int SERVERPUSH_TYPE_MEDIASERVERLIST = 1;
    public static final int SERVERPUSH_TYPE_USERLOGLEVEL = 2;
    public static final String SIMPLEACCOUNT_TAG_IsShare = "__isShare";
    public static final String SIMPLEACCOUNT_TAG_NICKNAME = "__nickName";
    public static final int SOCKETSTATE_ALLFAILED = 3;
    public static final int SOCKETSTATE_CLOSED = 1;
    public static final int SOCKETSTATE_OPENED = 2;
    public static final int SOCKETSTATE_RECEIVEDRESP = 4;
    public static final int SOCKETSTATE_UNKNOWN = 0;
    public static final String SP_PUSH_MANAGER = "sp_push_manager";
    public static final String SSO_ACCOUNT_ACTION = "ssoAccountAction";
    public static final int SSO_BAN_STATE_FORBIT_MAX = -10200;
    public static final int SSO_BAN_STATE_FORBIT_MIN = -10300;
    public static final int SSO_BAN_STATE_FORBIT_WRITE = -10201;
    public static final int SSO_CODE_APPID_INVALID = -10117;
    public static final int SSO_CODE_BLACK_IPV6_CMD = -10112;
    public static final int SSO_CODE_CMD_NOT_CONFIGED = -10115;
    public static final int SSO_CODE_CONN_FULL = -302;
    public static final int SSO_CODE_D2KEY_NOT_EXISTED = -10004;
    public static final int SSO_CODE_D2_EXPIRED = -10001;
    public static final int SSO_CODE_D2_REQUIRED = -10005;
    public static final int SSO_CODE_FORCE_QUIT = -10104;
    public static final int SSO_CODE_FORCE_QUIT_SDK = -10105;
    public static final int SSO_CODE_FORCE_QUIT_SERVER_PARSE_FAIL = -10106;
    public static final int SSO_CODE_INVALID_COOKIE = -10009;
    public static final int SSO_CODE_INVALID_D2 = -10003;
    public static final int SSO_CODE_NEED_A2_AND_IMEI = -10008;
    public static final int SSO_CODE_NEW_CONN_FAILD = -10114;
    public static final int SSO_CODE_NOT_WHITE_IPV6_CMD = -10111;
    public static final int SSO_CODE_NOT_WHITE_IPV6_UIN = -10110;
    public static final int SSO_CODE_OLD_CONN_FULL = 302;
    public static final int SSO_CODE_PARSE_FAILED = -10118;
    public static final int SSO_CODE_RS_NO_INSTANCE = -10116;
    public static final int SSO_CODE_SSO_EXPIRED_TOKEN = -10103;
    public static final int SSO_CODE_SSO_POP_TIPS = -10101;
    public static final int SSO_CODE_SSO_SUSPEND = -10102;
    public static final int SSO_CODE_UIN_NOT_MATCH = -10006;
    public static final int SSO_CODE_V20_DISABLE = -10119;
    public static final int SSO_CODE_VERIFY_CODE = -10000;
    public static final int SSO_CODE_VERIFY_CODE_TIME_OUT = -10007;
    public static final int SSO_CODE_ZLIB_UNCOPRESS_ERROR = -10107;
    public static final int SSO_PROTO_V0 = 0;
    public static final int SSO_PROTO_V10 = 10;
    public static final int SSO_PROTO_V11 = 11;
    public static final int SSO_PROTO_V20 = 20;
    public static final int SSO_PROTO_V21 = 21;
    public static final String TIMESTAMP_ADDSENDQUEUE = "__timestamp_addSendQueue";
    public static final String TIMESTAMP_APP2MSF = "__timestamp_app2msf";
    public static final String TIMESTAMP_MSF2APP = "__timestamp_msf2app";
    public static final String TIMESTAMP_MSF2NET = "__timestamp_msf2net";
    public static final String TIMESTAMP_MSF2NET_BOOT = "__timestamp_msf2net_boot";
    public static final String TIMESTAMP_MSF2NET_FOR_WEAKNET = "__timestamp_msf2net_for_weaknet";
    public static final String TIMESTAMP_NET2MSF = "__timestamp_net2msf";
    public static final String TIMESTAMP_NET2MSF_BOOT = "__timestamp_net2msf_boot";
    public static final String TIME_MSF2NET_BOOT_SHARP_C2S = "__timestamp_msf2net_boot_sharp_c2s";
    public static final String TPS_TELEMETRY_TRACING_INFO = "tps_telemetry_tracing_info";
    public static final int TYPE_BOOT_ACTION_KEEP_ALIVE = 1;
    public static final int TYPE_BOOT_ACTION_NORMAL = 0;
    public static final byte UINTYPE_QQ = 0;
    public static final byte UINTYPE_QQ_CONNECT_COMM = 6;
    public static final byte UINTYPE_QQ_GID_FOR_TEA_KEYEMP = 4;
    public static final String UIN_NOUIN = "0";
    public static final String UNKNOWN_SSID_STRING = "<unknown ssid>";
    public static int USEWAKELOCK_MINFAILEDCOUNT = 5;
    public static final String VAL_IPv4 = "IPv4";
    public static final String VAL_IPv4v6 = "v4v6";
    public static final String VAL_IPv6 = "IPv6";
    public static final String VAL_NONE = "none";
    public static String[] WiFiloginMergeTSAddress = null;
    public static String[] XGloginMergeTSAddress = null;
    public static int checkExpiresLogScreenOffCount = 2;
    public static boolean isUseDebugSso = false;
    public static long reSendIntrevTime = 5000;
    public static final String revision = "88c475c0";
    public static final String testServerAddress = "socket://113.96.213.95:14000";
    public static final String CMD_TRPC_MSG_PUSH = "trpc.msg.olpush.OlPushService.MsgPush";
    public static final String CMD_SEC_DISPATCH_TO_APP = "cmd_sec_dispatch_to_app";
    public static String[] CMD_NeedBootPushCmdHeads = {"VideoSvc", "SharpSvr.s2c", "MessageSvc.Push", "OnlinePush", CMD_TRPC_MSG_PUSH, CMD_SEC_DISPATCH_TO_APP};
    public static final String CMD_MSG_PBSENDMSG = "MessageSvc.PbSendMsg";
    public static String[] CMD_NeedResendCmds = {CMD_MSG_PBSENDMSG, "MessageSvc.SendMsg", "MessageSvc.offlinemsg", "MessageSvc.SendGroupMsg", "MsgProxy.SendMsg"};
    public static final int MSF_CONFIG_MANAGER = c.f250208c;
    public static final int MSF_SAMPLE_RATE_CONFIG_MANAGER = c.f250209d;
    public static final int MSF_UDP_CONFIG_MANAGER = c.f250210e;
    public static final int MSF_MULTI_TCP_CONFIG_MANAGER = c.f250211f;
    public static final int MSF_WAK_LOCK_CONFIG_MANAGER = c.f250218m;
    public static final int MSF_CORE_MULTI_TCP_CONFIG_MANAGER = c.f250212g;
    public static final int MSF_CORE_PACKET_COMBINE_CONFIG_MANAGER = c.f250213h;
    public static final int MSF_CORE_PKG_STATISTICS_CONFIG_MANAGER = c.f250214i;
    public static final int MSF_CORE_SIDE_WAY_HTTP_CONFIG_MANAGER = c.f250215j;
    public static final int MSF_CORE_SIDE_WAY_UDP_CONFIG_MANAGER = c.f250216k;
    public static final int MSF_CORE_TCP_ENGINE_CONFIG_MANAGER = c.f250217l;
    public static final int MSF_CORE_MONITOR_CONFIG_MANAGER = c.f250219n;
    public static final int MSF_CORE_MODIFY_CONFIG_MANAGER = c.f250220o;
    public static final int MSF_CORE_PKG_COMPRESS_CONFIG_MANAGER = c.f250221p;
    public static final int MSF_CORE_SSID_CONFIG_MANAGER = c.f250222q;
    public static final int MSF_CORE_NETWORK_CONFIG_MANAGER = c.f250223r;
    public static final int MSF_CORE_QUIC_ENGINE_CONFIG_MANAGER = c.f250224s;
    public static final int MSF_CORE_BEACON_SAMPLE_RATE_CONFIG_MANAGER = c.f250225t;
    public static final int MSF_PROBE_CONFIG_MANAGER = c.f250227v;
    public static final int MSF_CORE_DUAL_ENGINE_CONFIG_MANAGER = c.f250226u;
}
