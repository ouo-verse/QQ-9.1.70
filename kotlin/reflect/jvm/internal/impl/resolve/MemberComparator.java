...
public class MemberComparator implements Comparator<DeclarationDescriptor> {
    ...
    public int compare(DeclarationDescriptor declarationDescriptor, DeclarationDescriptor declarationDescriptor2) {
        ...
        if ((declarationDescriptor instanceof TypeAliasDescriptor) && (declarationDescriptor2 instanceof TypeAliasDescriptor)) {
            ...
            for (int i3 = 0; i3 < Math.min(valueParameters.size(), valueParameters2.size()); i3++) {
                DescriptorRenderer descriptorRenderer3 = RENDERER;
                int compareTo3 = descriptorRenderer3.renderType(valueParameters.get(i3).getType()).compareTo(descriptorRenderer3.renderType(valueParameters2.get(i3).getType()));
                if (compareTo3 != 0) {
                    return compareTo3;
                }
            }
            ...
            for (int i16 = 0; i16 < Math.min(typeParameters.size(), typeParameters2.size()); i16++) {
                List<KotlinType> upperBounds = typeParameters.get(i16).getUpperBounds();
                List<KotlinType> upperBounds2 = typeParameters2.get(i16).getUpperBounds();
                int size2 = upperBounds.size() - upperBounds2.size();
                if (size2 != 0) {
                    return size2;
                }
                for (int i17 = 0; i17 < upperBounds.size(); i17++) {
                    DescriptorRenderer descriptorRenderer4 = RENDERER;
                    int compareTo4 = descriptorRenderer4.renderType(upperBounds.get(i17)).compareTo(descriptorRenderer4.renderType(upperBounds2.get(i17)));
                    if (compareTo4 != 0) {
                        return compareTo4;
                    }
                }
