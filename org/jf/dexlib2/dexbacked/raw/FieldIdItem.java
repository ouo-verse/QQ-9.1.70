...
public class FieldIdItem {
    ...
    public static String asString(@Nonnull DexBackedDexFile dexBackedDexFile, int i3) {
        int offset = dexBackedDexFile.getFieldSection().getOffset(i3);
        return String.format("%s->%s:%s", dexBackedDexFile.getTypeSection().get(dexBackedDexFile.getBuffer().readUshort(offset + 0)), dexBackedDexFile.getStringSection().get(dexBackedDexFile.getBuffer().readSmallUint(offset + 4)), dexBackedDexFile.getTypeSection().get(dexBackedDexFile.getBuffer().readUshort(offset + 2)));
    }
    ...
    public static String getReferenceAnnotation(@Nonnull DexBackedDexFile dexBackedDexFile, int i3) {
        try {
            return String.format("field_id_item[%d]: %s", Integer.valueOf(i3), asString(dexBackedDexFile, i3));
        } catch (Exception e16) {
            e16.printStackTrace(System.err);
            return String.format("field_id_item[%d]", Integer.valueOf(i3));
        }
    }

    @Nonnull
    public static SectionAnnotator makeAnnotator(@Nonnull DexAnnotator dexAnnotator, @Nonnull MapItem mapItem) {
        return new SectionAnnotator(dexAnnotator, mapItem) { // from class: org.jf.dexlib2.dexbacked.raw.FieldIdItem.1
