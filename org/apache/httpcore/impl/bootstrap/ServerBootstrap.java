...
public class ServerBootstrap {
    ...
    public HttpServer create() {
        ...
        if (httpProcessor == null) {
            ...
            if (linkedList2 != null) {
                Iterator<HttpResponseInterceptor> it5 = linkedList2.iterator();
                while (it5.hasNext()) {
                    create.addFirst(it5.next());
                }
            }
            ...
            if (linkedList3 != null) {
                Iterator<HttpRequestInterceptor> it6 = linkedList3.iterator();
                while (it6.hasNext()) {
                    create.addLast(it6.next());
                }
            }
            LinkedList<HttpResponseInterceptor> linkedList4 = this.responseLast;
            if (linkedList4 != null) {
                Iterator<HttpResponseInterceptor> it7 = linkedList4.iterator();
                while (it7.hasNext()) {
                    create.addLast(it7.next());
                }
            }
            httpProcessor = create.build();
        }
        HttpProcessor httpProcessor2 = httpProcessor;
        ?? r16 = this.handlerMapper;
        if (r16 == 0) {
            r16 = new UriHttpRequestHandlerMapper();
            Map<String, HttpRequestHandler> map = this.handlerMap;
            if (map != null) {
                for (Map.Entry<String, HttpRequestHandler> entry : map.entrySet()) {
                    r16.register(entry.getKey(), entry.getValue());
                }
            }
        }
        HttpRequestHandlerMapper httpRequestHandlerMapper = r16;
